...
def test_phones_at_home_page(app):
    ...
    assert contact_to_check_from_home_page.all_phones_from_home_page == app.contact.merge_phones(contact_from_edit_page)
    #assert for email addresses
    assert contact_to_check_from_home_page.all_emails == app.contact.merge_emails(contact_from_edit_page)
    #first name
